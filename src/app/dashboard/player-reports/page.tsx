...
export default function PlayerReportsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (user?.role === 'STAFF') {
        ...
        const response = await fetch('/api/player-reports/staff-reports', {
          headers: {
        ...
        if (staffResponse.ok) {
          const playersData = await staffResponse.json()
          setStaff(Array.isArray(playersData) ? playersData : [])
        }
      }
    } catch (error) {
  ...
  const fetchReports = async (folderId?: string) => {
    try {
      ...
      if (user?.role === 'STAFF') {
        // For staff users, use the staff-reports API
        const url = folderId 
          ? `/api/player-reports/staff-reports?folderId=${folderId}`
          : '/api/player-reports/staff-reports'
  ...
  const handleUploadReport = async (name: string, description: string, file: File) => {
    try {
      ...
      console.log('Sending request to /api/player-reports...')
        ...
        {folders.filter(folder => folder.parentId === (selectedFolder?.id || null)).map((folder) => (
          ...
          >
            ...
            <div className="flex justify-between text-xs" style={{ color: colorScheme.textSecondary }}>
              <span>{(folder.reports?.length || folder._count?.reports || 0)} reports</span>
              <span>{(folder.children?.length || folder._count?.children || 0)} folders</span>
            </div>
          </div>
        ))}
      </div>
